...
const EventCard = ({ event, bannerEvent = false }:IEventCard) => {
    console.log(event)
    const documentByEvent = eventDocuments.filter((document)=> document.idEvent === event.id)
  ...
  return (
    <div className={`event-card`}>
      <img
        ...
        src={documents.filter((document)=> document.id === documentByEvent[0].idDocument)[0].url}
        alt={documents.filter((document)=> document.id === documentByEvent[0].idDocument)[0].name}></img>
      <div className="event-card__informations">
        <div className="event-card__informations__header">
          ...
          <span
            ...
            {eventPostType[0].name === 'Activité' ? eventActivity[0].name : eventPostType[0].name}
          </span>
        </div>
